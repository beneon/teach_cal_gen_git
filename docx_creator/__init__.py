...
class DocxUpdate:
    def __init__(self,tag_dict:dict,identifier:str,template_path:str, dunder_included:bool=False, output_path:str=""):
        """
    ...
    def create_row(self,tbl_id,content_list):
        tbl = self.doc.tables[tbl_id]
        row_cells = tbl.add_row().cells
        assert len(row_cells)==len(content_list), f'row have {len(row_cells)} cells, yet content have {len(content_list)} element'
        for i,c in enumerate(content_list):
            rc = row_cells[i]
            rc.paragraphs[0].text=c

    def doc_save(self):
        self.doc.save(os.path.join(self.output_path,f'{self.identifier}.docx'))
